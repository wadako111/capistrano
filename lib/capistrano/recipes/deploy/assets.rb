...
namespace :deploy do
  namespace :assets do
    ...
    DESC
    task :symlink, :roles => lambda { assets_role }, :except => { :no_release => true }, :on_no_matching_servers => :continue do
      run <<-CMD.compact
    ...
    DESC
    task :precompile, :roles => lambda { assets_role }, :except => { :no_release => true }, :on_no_matching_servers => :continue do
      run <<-CMD.compact
        cd -- #{latest_release} && 
    ...
    DESC
    task :update_asset_mtimes, :roles => lambda { assets_role }, :except => { :no_release => true }, :on_no_matching_servers => :continue do
    ...
    desc <<-DESC
      ...
      asset_env variable. The defaults are:
        ...
        set :asset_env, "RAILS_GROUPS=assets"
    DESC
    task :clean, :roles => lambda { assets_role }, :except => { :no_release => true }, :on_no_matching_servers => :continue do
    ...
    desc <<-DESC
      ...
      an existing release.
    DESC
    task :clean_expired, :roles => lambda { assets_role }, :except => { :no_release => true }, :on_no_matching_servers => :continue do
    ...
    DESC
    task :rollback, :roles => lambda { assets_role }, :except => { :no_release => true }, :on_no_matching_servers => :continue do
